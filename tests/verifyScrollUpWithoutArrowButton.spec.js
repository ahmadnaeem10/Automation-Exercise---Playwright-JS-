...
test('Test Case 26: Verify Scroll Up without Arrow button and Scroll Down functionality', async ({ page }) => {
    ...
    // Step 6: Scroll up page to top (without using arrow button)
    await page.evaluate(() => {
        window.scrollTo(0, 0);
    });
    
    // Wait for the scroll operation to complete using a non-timeout approach
    await page.waitForLoadState('networkidle');
    ...
    if (!textFound) {
        ...
        console.log(`Scroll position after scrolling up: ${scrollPosition}px from top`);
    ...
    console.log('Test Case 26 completed successfully: Verified Scroll Up without Arrow button and Scroll Down functionality');
});